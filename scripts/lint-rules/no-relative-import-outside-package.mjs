    ...
    return {
      ImportDeclaration: function (node) {
        ...
        if (failPackage) {
          context.report({
            node,
            message: `Import of "${importPath}" reaches outside of the package "${failPackage}". Import it by its published package name instead.`,
            fix(fixer) {
              ...
              if (packageName) {
                ...
                let absolutePath = importPath.replace(/(?:\.\.\/)+.*?\//, `${packageName}/`);
      ...
      CallExpression: function (node) {
        if (node.callee.name === "require") {
          ...
          if (failPackage) {
            context.report({
              node,
              message: `Require of "${requirePath.value}" reaches outside of the package "${failPackage}". Import it by its published package name instead.`,
              fix(fixer) {
                // Extract the package name from the relative path
