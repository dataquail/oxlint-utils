...
Every glob in a YAML manifest is quoted. A bare `*` opens an alias, a bare `@` or backtick
...
Double quotes process backslash escapes, so a regex with `\d` or `\.` goes in single quotes,
...
how a long message or a [probe source](/goodbones/architecture-rules/enforcement/probes/)
