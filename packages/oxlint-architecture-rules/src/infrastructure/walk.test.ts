...
import { typescriptLanguage } from "./languages/typescript/index.js";
...
describe("listSourceFiles", () => {
  it("finds the sources under a root", () => {
    const files = listSourceFiles(
      repoRoot,
      ["packages/oxlint-architecture-rules/src/core"],
      TYPESCRIPT,
    );
  ...
  it("skips the folders no policy is written about", () => {
    expect(
      listSourceFiles(repoRoot, ["packages/oxlint-architecture-rules"], TYPESCRIPT).some((f) =>
...
describe("roots", () => {
  ...
  it("takes a root that names a file as the file itself", () => {
    expect(
      listSourceFiles(
        repoRoot,
        ["packages/oxlint-architecture-rules/src/core/baseline.ts"],
        TYPESCRIPT,
      ),
